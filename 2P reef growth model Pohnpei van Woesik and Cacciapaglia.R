...
rtd<-reef.transectdata<-read_csv("E:/ACCRETION OF REEFS/Accretion model 2018/Accretion Pohnpei Kosrae 2018/Pohnpei/Pohnpei coral data June 24 2018 RvW final.csv")
...
sumdata$Species[which(sumdata$Species=='Heliopora')]<-'Heliopora sp.'
sumdata$Species[which(sumdata$Species=='Heliofungia')]<-'Heliofungia sp.'
sumdata$Species[which(sumdata$Species=='Montipora')]<-'Montipora sp.'
sumdata$Species[which(sumdata$Species=='Physogyra')]<-'Physogyra sp.'
sumdata$Species[which(sumdata$Species=='Plerogyra')]<-'Plerogyra sp.'
sumdata$Species[which(sumdata$Species=='Diploastrea')]<-'Diploastrea heliopora'
sumdata$Species[which(sumdata$Species=='Ctenactis')]<-'Ctenactis sp.'
sumdata$Species[which(sumdata$Species=='Coscinaraea')]<-'Coscinarea sp.'
sumdata$Species[which(sumdata$Species=="Goniastrea aspera")]<-"Coelastrea  aspera"
sumdata$Species[which(sumdata$Species=="Goniastea retiformis")]<-"Goniastrea retiformis"
sumdata$Species[which(sumdata$Species=="Galaxea fasicularis")]<-"Galaxea fascicularis"


sumdata$Species<-gsub('Favia','Dipsastrea',sumdata$Species)
sumdata$Species<-gsub('Montastrea','Astrea',sumdata$Species,ignore.case = FALSE)
rtds<-sumdata
...
slist<-slist[-(grep('Sinularia',slist,ignore.case = TRUE))]
...
BFj<-numeric()
BUj<-numeric()
pfspeceros<-matrix(ncol=24,nrow=24)
for (j in 1:(length(sites))){
  ...
  ftd<- read_excel("fishes Pohnpei 2018 june 21 final.xlsx") #will change directory name based on island later
  ...
  ####################### URCHIN BIOEROSION
  utd<-urchin.transectdata<-read.csv("urchins Pohnpei 2018 june 17 final.csv")
...
palsites <- read.csv("Pohnpei sitesCC.csv" )
...
#write.csv(islandDF,'pohnpei site model3.csv')##################################################################################
...
dev.off()
par(mar=c(12,4,2,0))
barplot(sumplot2$sum, ylab='Pohnpei cumulative meters',names.arg=sumplot2[,1],ylim=c(0,450),cex.names=.9,cex.lab=1.2,las=2,yaxt="n"); axis(2, las=3)
...
tiff('Pohnpei carb prod.tif',width=4000,height=1600,res=300)
par(mar=c(10,5,2,1))
barplot(cbinesum$sumcp,names.arg=namearg,cex.names=.9,cex.lab=1.3,las=2,ylab=expression(paste('Gross carbonate production (kg',m^2,yr^-1,')')))
