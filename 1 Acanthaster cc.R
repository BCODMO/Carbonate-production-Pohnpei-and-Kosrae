...
# > 40 cm Acanthaster planci can eat 161 (winter) and 357 to 478 cm2 day in summer
#20-39 cm Acanthaster planci can eat about 66 cm2 day (winter) to 234 cm2 day in summer
#Let's assume they eat 100 cm2 day on average, which is 0.01 m2 day, or 3.65 m2 year
#Our estimates were 10 m by 5 m tranecsts (50 m2)

# > 40 cm Acanthaster planci can eat 161 (winter) and 357 to 478 cm2 day in summer
#20-39 cm Acanthaster planci can eat about 66 cm2 day (winter) to 234 cm2 day in summer
#Let's assume they eat 100 cm2 day on average, which is 0.01 m2 day, or 3.65 m2 year
#Keesing and Lucas 1992

n<-1000
size<-rbeta(n,5,2)*45 #hypothetical size distribution of coral grazing starfish
sized<-density(size)
plot(sized)
#half the year summer, half winter... at the equator? 
large<-sum(size>=40)*mean(357,478)
med<-length(size[size<39&size>20])*mean(66,234)
sum(med,large)/n
#WOW, your estimate of 100cm2 was sooo close to this approach's estimate

#get average size of coral.
#LCClist<-rtds[rtds$Species%in%spplist,]
#mLCC<-mean(LCClist$planar.proportion)*10*100 #from proportion of 10m to cm #average linear distance (cm)
mLCC<-21.04238  # average size of coral in linear cm.
#assume circular corals/coral stands. 
avgA<-(mLCC/2)^2*pi  #Average size of coral stand on Kosrae is 348cm^2

#so that's roughly 1 coral every 3.5 days... handling time should be ~84hrs
#attack rate should be based on resource density. How quickly it can get to another coral... 
speed<-35#cm min-1 #muller et al 2011, average speed of A.planci
speed<-speed/100*1440 #convert to m d-1; ~500m d-1


# LCClist<-rtds[rtds$Species%in%spplist,]
# mLCC<-ddply(LCClist, c("Site"), summarise,
#             meanLCC=sum(planar.proportion))
# mean(mLCC$meanLCC) #31% average live coral cover is 31%


#very simple model, the output should be in the same ball park as this estimate.
Acan<-2.3 #number of acanthaster
con<-0.01 #100cm2/d
days<-365 #to year
#con*days 3.65 m2/yr... for one average size A.planci #solid base to start here. roughly a 2x2m area. or ~1/12 of our transect. (so if R is < 0.1 it'll remove all coral production...)
Density<-1.4*10 #(kg/m2) #density in 1 g/cm3 -> 10000/1000 kg/m2 x10 ~We want kg/m2*yr (an impossible unit) but kg/m3 is a consider cm3 to m2 as flat
transectsz<-50
#R<-0.2 #proportion of transect available for consumption, should actually just be a limit in this model(max consumption negates GP), and used for rate of ingestion -use for rate
Acan*con*days*Density/transectsz
...
#loop for multiple Acan model and add RI scalar
dev.off()
plot(R,Acanthasteri,xlab='resource density',ylab='kgCaCO3.m-2.yr-1 carbonate potential removed',type='l',col=1,ylim=c(0,10))
for (i in seq(1,4,.1)){
  R=seq(1,100, by=1) #Resource density
  h=3.5 # handling rate (days/coral)
  #85 (hrs/coral)
  
  #attack rate d^-1 (Resource Density/transect size/speed of A.planci) #higher resource density faster attack rate
  #how many corals an A.planci can attack/day without handling, they move fast while not eating, most time is spent consuming.
  a=12/((((100-R+1)/20)^2*pi)*50/speed) #(corals/day)
  #12 hrs a day of eating / distance between corals based on resource density(R) +1 to avoid 0, / 2 ^2 pi for area of circle to search between corals* size of transect (m2) / speed at which A.planci travel. == number of corals an individual can reach in a day based on resource density(R)
  Acan = i #number of Acanthaster
  Density=1.42
  con=0.01 #100 cm2 day on average .01 m2.d-1 #assume average  rate is at average resource density... LCC/R
  tcon<-Acan*con #number of A.planci * consumption rate (m2.d-1)
  per.transect<-tcon*Density*10*365 #include density (g/cm3), convert to kgm-2 (10) * consumption: kg.m2/m3.d = kg/m2*yr
  
  avgLCC<-31  # Kosrae average LCC is 31%. Assume this is where the average consumption rates (0.01) of A.planci were observed.
  
  
  # Only difference from very simple model 
  RI1= (a*R/100)/(1+a*h*R/100) #attack rate (corals/d), resource density (0-1), handling rate (hrs) #Rate of coral ingestion, based on resource density m^2.d^-1 
  rescale<-1/RI1[avgLCC]
  RI= (a*R/100)/(1+a*h*R/100)*rescale #rescale based on the max consumption at average resource density.
  #rescale<-4.53
  
  
  Acanthasteri = (per.transect * RI)/50 #kg/m2 *Rate of coral ingestion (unitless scalar), over 50 sq meters observed in a transect
  #50m2 in the transect, but they will keep reducing reefs past the transect boundaries...
  if(i==2.3){lines(R,Acanthasteri,col=i,lwd=3)} #our max number of COT/6 for transect 
  lines(R,Acanthasteri,col=i,lwd=1)
}
...
######### incorporate into model with above calculated rates and observed COT and coral distributions..
...
sumdata <- ddply(rtd, c("Site","Species"), summarise,
                 ...
                 COT=sum(as.numeric(COT),na.rm=T))
...
large<-sum(size>=40)*mean(357,478)
med<-length(size[size<39&size>20])*mean(66,234)
consumptionrate<-sum(med,large)/n  #cm^2 per day average
#estimate of 100cm2 was super close
...
#so that's 1 coral every 3.5 days... handling time should be ~84
# average size of coral in linear cm (diameter).
#assume circular corals/coral stands. 
avgA<-(mLCC/2)^2*pi  #Average size of coral stand on Kosrae is 348cm^2


h<-avgA/consumptionrate #handling rate is around 85 hrs for the average coral.
speed<-35#cm min-1 #muller et al 2011, average speed of A.planci
speed<-speed/100*1440 #convert to m d-1; ~500m d-1
...
for (j in 1:(length(sites))){
  ...
  Dlist<-NA;extensionratematch<-NA;Densitymatch<-NA;morphologies<-NA;Lklist<-NA;xklist<-NA
  ...
  Acanthasteri<-NA 
  siteLCC<-sum(xklist,na.rm=T)
  avgDens<-sum(Dlist*xklist/siteLCC,na.rm=T)
  
  
  R=siteLCC*100 #Resource density
  h=h # handling rate 
  a=12/((((100-R+1)/20)^2*pi)*50/speed) #attack rate d^-1
  #12 hrs a day of eating / distance between corals based on resource density(R) +1 to avoid 0, / 2 ^2 pi for area of circle to search between corals* size of transect (m2) / speed at which A.planci travel. == number of corals an individual can reach in a day based on resource density(R)
  days<-365 #to year
  #con*days 3.65 m2/yr... for one average size A.planci #solid base to start here. roughly a 2x2m area. or ~1/12 of our transect. (so if R is < 0.1 it'll remove all coral production...)
 #(kg/m2) #density in 1 g/cm3 -> 10000/1000 kg/m2 x10 ~We want kg/m2*yr (an impossible unit) but kg/m3 is a consider cm3 to m2 as flat
  transectsz<-50
  con=0.01 #100 cm2 day on average .01 m2.d-1 #assume average  rate is at average resource density... LCC/R
  
  Acan = sum(rtdsj$COT)/6 #number of Acanthaster/6 transects
  #the number of Acan is absolute, their range of infulence depends on the density of corals.  2.8 m. d - ~ at Davies Reef (low density starfish/highcoral cover) and 10.3 m. d-1  so in low densities, one starfish influences 10 linear meters. and at high density it influences 3m (width of influence could be average Acan diameter)30cm? 

  tcon<-Acan*con #number of A.planci * consumption rate (m2.d-1)
  #include density (g/cm3), convert to kgm-2 (10) * consumption: kg.m2/m3.d = kg/m2*yr

  # Kosrae average LCC is 31%. Assume this is where the average consumption rates (0.01) of A.planci were observed.
  rescale<-4.53 #based on COT simple
  ##attack rate (corals/d), resource density (0-1), handling rate (hrs) #Rate of coral ingestion, based on resource density m^2.d^-1 
  RI= (a*R/100)/(1+a*h*R/100)*rescale #rescale based on the max consumption at average resource density.
  
  Density=avgDens # average coral density to inject kg into the equation... kg/m3 include actual average per transect. #(kg/m2) #density in 1 g/cm3 -> 10000/1000 kg/m2 x10 ~We want kg/m2*yr (an impossible unit) but kg/m3 is a consider cm3 to m2 as flat
  per.transect<-tcon*Density*10*365 #include density (g/cm3), convert to kgm-2 (10) * consumption: kg.m2/m3.d = kg/m2*yr
  
  Acanthasteri = (per.transect * RI)/transectsz # summed over i,j, n
  ...
  cbineCA<-rbind(cbineCA,c('COT',Acanthasteri,avgDens,rep(NA,4)))
  pcbine<-cbineCA
...
## negative sedimentation effect for southern Kosrae, Utwe Bay
sed[23]<-(-0.4)
...
NP<-(GP*r)-COT  #remove COT erosion only from Gross Production since it isn't eroding bare carbonate. 

NP[NP<0]<-0

NP<-NP+sed-(BFj+BUj+BM)
...
islandDF<-data.frame(P.locat,NP,GP*r,BFj,BUj,palsites$real.lon,palsites$real.lat,COT)
...
write.csv(islandDF,'Kosrae site model COT3.csv')##################################################################################
...
barplot(cbinesum$sumcp,names.arg=cbinesum[,1],cex.names=.9,cex.lab=1,las=2,ylab='Carbonate production (kg.m^2.yr^-1)')
