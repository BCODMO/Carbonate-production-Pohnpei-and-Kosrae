...
PptsCOT <- read_csv("pohnpei site model COT3.csv",col_types = cols(X1 = col_skip()))

coordinates(Ppts)<-~palsites.real.lon+palsites.real.lat
coordinates(PptsCOT)<- ~palsites.x+palsites.y




Ppts$NP<-Ppts$NP-PptsCOT$COT     # real lat lon and just subtract the COT values from the NP
...
writeRaster(Pmsk1,'Pohn_outerCOT.nc',overwrite=TRUE)
writeRaster(Pmsk2,'Pohn_innerCOT.nc',overwrite=TRUE)
writeRaster(Pmsk3,'Pohn_patchCOT.nc',overwrite=TRUE)
# 
...
#########################################
setwd("E:/ACCRETION OF REEFS/Accretion model 2018/Accretion Pohnpei Kosrae 2018/Figures/Pohnpei")
pouter<-raster('Pohn_outerCOT.nc')
pinner<-raster('Pohn_innerCOT.nc')
ppatch<-raster('Pohn_patchCOT.nc')


#quick fix
po<-raster('Pohn_outer.nc')
pi2<-raster('Pohn_inner.nc')
pp<-raster('Pohn_patch.nc')

pouter<-mask(pouter,po)
pinner<-mask(pinner,pi2)
ppatch<-mask(ppatch,pp)
...
e<-c(158.050,158.43,6.74,7.07) #e<-c(158.050,158.3855,6.74,7.07)
...
library(readr)
PptsLL <- read_csv("E:/ACCRETION OF REEFS/Accretion model 2018/Accretion Pohnpei Kosrae 2018/Pohnpei/Pohnpei site model COT3.csv",col_types = cols(X1 = col_skip()))
coordinates(PptsLL)<- ~palsites.x+palsites.y
...
tiff('Pohnpei net reef accretion krige COTtest.png',width=551*sz,height=482*sz, res = 300)
plot(map)
par(mar=c(0,0,0,0))
library(plotrix)
...
allr<-c(ar,br,cr)
##################################################################
image(pinner,maxpixels=500000,col=cols,zlim=range(allr),xlab='',ylab='',add=T) #, xaxs='r',yaxs = 'r',col.axis='white'
color.legend(162.9109,5.366731,162.9509,5.371731,legend=round(seq(0,range(allr)[2],length.out=5),0), rect.col=cols,cex=1,col='white')
image(ppatch,add=T,col=cols,zlim=range(allr),colorbar=F)
image(pouter,add=T,col=cols,zlim=range(allr),colorbar=F)
text(158.1279,7.03961,'Net carbonate production',col='white')
text(158.1279,7.0291,expression(paste('(kg CaCO'[3],' m'^'-2',' yr'^'-1',')',sep='')),col='white')

points(PptsLL,cex=PptsLL@data$COT/1.5+.4,pch=1,lwd=3,col='white')
points(PptsLL,cex=PptsLL@data$COT/1.5,pch=1,lwd=3,col='black')
...
#legend
points(158.32,7.05,cex=5/1.5+.4,lwd=3,col='white')
points(158.32,7.05,cex=5/1.5,pch=1,lwd=3,col='black')

points(158.32,7.033,cex=3/1.5+.4,lwd=3,col='white')
points(158.32,7.033,cex=3/1.5,pch=1,lwd=3,col='black')

points(158.32,7.022,cex=1/1.5+.4,lwd=3,col='white')
points(158.32,7.022,cex=1/1.5,pch=1,lwd=3,col='black')

text(158.38,7.05,expression(paste("-5 kg CaCO"[3],' ',m^-2,' ',yr^-1)),cex=.8,col='white')
text(158.377,7.035,expression(paste("-3 kg CaCO"[3],' ',m^-2,' ',yr^-1)),cex=.8,col='white')
text(158.374,7.022,expression(paste("-1 kg CaCO"[3],' ',m^-2,' ',yr^-1)),cex=.8,col='white')
##################################################################
compassRose(158.083,6.815095,col='white',cex=.76)
color.legend(158.0858,7.04561,158.15,7.052,legend=round(seq(0,range(allr)[2],length.out=5),0), rect.col=cols,cex=1,col='white')
